...
import static dadkvs.server.Constants.BASE_PORT;
import static dadkvs.server.Constants.NUM_RESPONSES;
...
public class DadkvsClient {
    ...
    public DadkvsClient() {
        ...
        host = "localhost";
        sequence_number = 0;
        responses_needed = NUM_RESPONSES;
        rnd = new Random();
    ...
    private VersionedValue doRead(int key) {
        ...
        GenericResponseCollector<DadkvsMain.ReadReply> read_collector =
                new GenericResponseCollector<DadkvsMain.ReadReply>(read_responses, n_servers);
        ;

        read_request.setReqid(reqid).setKey(key);
        for (int i = 0; i < n_servers; i++) {
            CollectorStreamObserver<DadkvsMain.ReadReply> read_observer =
                    new CollectorStreamObserver<DadkvsMain.ReadReply>(read_collector);
        ...
        if (read_responses.size() >= responses_needed) {
            ...
            System.out.println(
                    "read key "
                            ...
                            + ">");
            VersionedValue kv_entry =
                    new VersionedValue(read_reply.getValue(), read_reply.getTimestamp());
            ;
            return kv_entry;
        } else {
    ...
    private void doTransactions() throws Exception {
        ...
        while (counter < loop_size) {
            ...
            if (doCommit(
                    read_key1,
                    kv_entry1.getVersion(),
                    read_key2,
                    kv_entry2.getVersion(),
                    write_key,
                    write_value)) committed++;
            Thread.sleep(rnd.nextInt(sleep_range) * 1000);
            counter++;
        }
        System.out.println(
                "loop done. transactions committed = "
                        + committed
    ...
    public void parseArgs(String[] args) {
        ...
        if (length < 1) {
            ...
            if (id == 0) System.err.println("Client id=0 reserved for console, using client id=1!");
            else client_id = id;
        }
        ...
        while (cursor < length) {
            ...
            switch (option_name) {
                case "--help":
                    ...
                    break;
                case "--port":
                    if (option_parameter == null) System.err.println("missing portnumber");
                    else port = Integer.parseInt(option_parameter);
                    break;
                case "--host":
                    if (option_parameter == null) System.err.println("missing hostname");
                    else host = option_parameter;
                    break;
                case "--range":
                    if (option_parameter == null) System.err.println("missing keyrange");
                    else key_range = Integer.parseInt(option_parameter);
                    break;
                case "--lenght":
                    if (option_parameter == null) System.err.println("missing looplenght");
                    else loop_size = Integer.parseInt(option_parameter);
                    break;
                case "--sleep":
                    if (option_parameter == null) System.err.println("missing sleeprange");
                    else sleep_range = Integer.parseInt(option_parameter);
                    break;
                case "-i":
                            ...
                            if (kv_entry != null)
                                System.out.println(
                                        "did read "
                                                ...
                                                + " and version "
                                                + kv_entry.getVersion());
                            else System.out.println("failed to read " + key);
                        } catch (NumberFormatException e) {
                            System.out.println("usage: read key");
                        }
                    } else System.out.println("usage: read key");
                    break;
                case "tx":
                    System.out.println(
                            "tx reading key "
                            ...
                            if (write_key == 0)
                                ...
                                if ((kv_entry1 != null) && (kv_entry2 != null))
                                    doCommit(
                                            ...
                                            write_key,
                                            write_value);
                                else System.out.println("failed to read keys");
                            }
                        } catch (NumberFormatException e) {
                            System.out.println("usage: read key");
                        }
                    } else System.out.println("usage: tx read_key read_key write_key");
                    break;
                case "lenght":
                    ...
                    } else System.out.println("usage: lenght loop-lenght");
                    break;
                case "range":
                    ...
                    } else System.out.println("usage: range key-range");
                    break;
                case "time":
                    ...
                    } else System.out.println("usage: time sleep-range");
                    break;
                case "loop":
                    try {
                        doTransactions();
    ...
    public void main_loop(String[] args) throws Exception {
        ...
        System.out.println(
                "Client id = " + client_id + " serverhost = " + host + " port = " + port);
        ...
        if (interactive_mode == false) doTransactions();
        else goInteractive();
